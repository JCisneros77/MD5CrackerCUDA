...
__device__ static char * getString(int start, int length, char * words){
  ...
  //printf("GPU STRING:%s\n",out);
    for(int i = 0; i < length; ++i)
      out[i] = words[start+i];
      //printf("%s",words[start+i]);
  //printf("\n");
   //printf("Out:%s\n",out);
return out;
}

__global__ static void MDString (char * words,int * hash_found,int * target_hash,int * indexes, int numberOfWords,int * wordLengths)
{
  // Get thread id
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  // Check if MD5 has been cracked
  if (idx == 0)
    hash_found[0] = 0;
  // If hash hasn't been found, get MD5 for next word
  if (idx < numberOfWords && hash_found[0] != 1){
    // Get string for this thread
    char * inString = getString(indexes[idx],wordLengths[idx],words);
    // Calculate MD5
    MD5_CTX mdContext;
...
extern "C" int callMD5CUDA(struct deviceInfo * device,char * words, int * target_hash,int * indexes, int numberOfWords,int * wordLengths) {
  ...
  // Call CUDA Function on GPU
	MDString <<< device->max_blocks, device->max_threads>>> (words, d_hash_found,target_hash,indexes,numberOfWords,wordLengths);
  // Stop Execution Time
  ...
  cudaMemcpy(h_hash_found, d_hash_found, sizeof(int), cudaMemcpyDeviceToHost );
  cudaFree(d_hash_found);
  int returnFlag = h_hash_found[0];
  free(h_hash_found);
  // Return flag hash found to CPU
  return returnFlag;
}
